...
- [ ] Recheck user activity immediately before sending so recently disabled users are skipped.
...
- [ ] Confirm retries reuse the same persisted recipient/local-date/card delivery identity and provider idempotency key.
...
- [ ] Confirm the next demo run starts from a zeroed aggregate delivery baseline after cleanup.
